...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        X = self.model_embeddings.source(source_padded) # source_padded = (src_len, b) / X = (src_len, b, e)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        Y = self.model_embeddings.target(target_padded) # target_padded = (tgt_len, b) / Y = (tgt_len, b, e)
        for Y_t in Y.split(1, dim=0):
            Y_t = Y_t.squeeze(0) # Y_t = (1, b, e) -> (b, e)
            Ybar_t = torch.cat([Y_t, o_prev], dim=1) # o_prev = (b, h)
            dec_state, o_t, e_t = self.step(Ybar_t, dec_state, enc_hiddens, enc_hiddens_proj, enc_masks) # combined_output = (b, h)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        e_t = enc_hiddens_proj.bmm(dec_hidden.unsqueeze(2)).squeeze(2) # e_t = (b, src_len) / enc_hiddens_proj = (b, src_len, h) / dec_hidden = (b, h) 
        ...
        alpha_t = F.softmax(e_t, dim=1) # e_t = (b, src_len) / alpha_t = (b, src_len)
        a_t = alpha_t.unsqueeze(1).bmm(enc_hiddens).squeeze(1) # a_t = (b, 2h) / alpha_t = (b, src_len) / enc_hiddens = (b, src_len, 2h)
        U_t = torch.cat([a_t, dec_hidden], dim=1) # a_t = (b, 2h) / dec_hidden = (b, h)
