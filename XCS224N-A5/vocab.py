...
class VocabEntry(object):
    ...
    def words2charindices(self, sents):
        ...
        return [[ [self.start_of_word] + [self.char2id.get(c, self.char_unk) for c in w] + [self.end_of_word] for w in s ] for s in sents]
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        ###     previous parts
        sents_padded_chars = pad_sents_char(self.words2charindices(sents), self.char2id['<pad>'])
        sents_var = torch.tensor(sents_padded_chars, dtype=torch.long, device=device).permute(1,0,2)
        return sents_var
        ### END YOUR CODE
