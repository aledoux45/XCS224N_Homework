...
def naiveSoftmaxLossAndGradient(
    ...
    # Calculate y_hat
    y_hat = np.zeros(num_words)
    for i in range(num_words):
        y_hat[i] = np.exp(np.dot(outsideVectors[i,:], centerWordVec)) / np.sum(np.exp(np.matmul(outsideVectors, centerWordVec)))
    # Calculate y 
    y = np.zeros(num_words)
    y[outsideWordIdx] = 1
    loss = -np.log(y_hat[outsideWordIdx])  # scalar
    gradCenterVec = np.matmul(y_hat - y, outsideVectors)   # size 1 x d
    gradOutsideVecs = np.outer(y_hat - y, centerWordVec)   # size V x d
...
def negSamplingLossAndGradient(
    ...
    gradOutsideVecsTemp = delta.reshape((K + 1, 1)).dot(centerWordVec.reshape((1, centerWordVec.shape[0])))
    for k in range(K + 1):
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    loss = 0.0
    gradCenterVecs = np.zeros(centerWordVectors.shape)  # size V x d
    gradOutsideVectors = np.zeros(outsideVectors.shape)  # size V x d

    for word in outsideWords:
        centerWordIdx = word2Ind[currentCenterWord]
        centerWordVec = centerWordVectors[centerWordIdx,:]
        outsideWordIdx = word2Ind[word]
        lossCurrent, gradCenterVec, gradOutsideVecs = word2vecLossAndGradient(centerWordVec, outsideWordIdx, outsideVectors, dataset)
        loss += lossCurrent
