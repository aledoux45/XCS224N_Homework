...
def computeCoOccurrenceMatrix(corpus, window_size=4):
    ...
    M = np.zeros((num_words, num_words), dtype=np.int_)
    word2Ind = {word:i for i,word in enumerate(words)}

    for doc in corpus:
        for w, word in enumerate(doc):
            i = word2Ind[word]
            # context = doc[max(0, w-window_size):min(len(doc),w+window_size)]
...
def reduceToKDim(M, k=2):
    ...
    """
    random_state = np.random.seed(4355)
    n_iters = 10  # Use this parameter in your call to `TruncatedSVD`
    print("Running Truncated SVD over %i words..." % (M.shape[0]))

    svd = TruncatedSVD(n_components=k, algorithm='randomized', n_iter=n_iters, random_state=random_state, tol=0.0)
    svd.fit(M)
